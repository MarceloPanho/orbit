...
<div style="{{ $shell }}">
    ...
    @unless($embedded)
        <x-ui.page-header title="Categorias" subtitle="Categorias das Rendas/Recebimentos" />
    @endunless

    <x-ui.card :title="$editingId ? 'Editar categoria' : 'Nova categoria'">
        <form wire:submit="saveIncomeCategory" style="display:flex; gap:16px; flex-wrap:wrap; align-items:flex-end;">
            <div style="flex:2; min-width:200px;">
                <x-ui.input label="Nome" type="text" placeholder="Ex.: Salário, Freelance..." wire:model="name" />
        ...
        <x-ui.table>
            ...
            <x-ui.tr wire:key="categories-{{ $category->id }}">
                ...
                <x-ui.td align="right" nowrap>
                    <x-ui.action-button icon="✎" wire:click="editIncomeCategory({{ $category->id }})">Editar</x-ui.action-button>
                    <x-ui.action-button icon="✕" tone="danger"
                        x-on:click="$dispatch('confirm', {
                            ...
                            confirmText: 'Excluir',
                            onConfirm: () => $wire.deleteIncomeCategory({{ $category->id }}),
